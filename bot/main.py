...
def start(message):
    ...
    bot.send_message(message.chat.id,
                     "Привет-привет! Меня зовут БОТ CARGO.RUN. Рад приветствовать тебя. Я помогу тебе найти ответы на твои вопросы. Что тебя интересует?", reply_markup=reply_markup, timeout=60)
...
3. В периоды пиковой нагрузки выгрузка листа может занять некоторое время, пока ваш запрос стоит в очереди на обработку, но он в любом случае будет выполнен."
    ...
    if message.text == "Вся информация":
